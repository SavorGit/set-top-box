...
public class WifiApUtil {
    ...
    public boolean setWifiApEnabled(boolean enabled) {
        ...
        try {
            //配置热点的名称
            String ssid = "RD";
//            if (!TextUtils.isEmpty(Session.get(context).getRoomName())) {
//                ssid = ssid + Session.get(context).getRoomName();
//            } else {
            ...
            if (!TextUtils.isEmpty(mac) && mac.length() > 3) {
                ...
                ssid = ssid + timestamp.substring(timestamp.length() - 5);
            }
//            }
            return turnOnWifiAp(ssid, "11111111", WifiSecurityType.WIFICIPHER_WPA2);
        } catch (Exception e) {
            return false;
        }
    }
