...
//        }
        ...
        for (int i = 0; i < mTexts.length; i++) {
            int n = (mMax - mMin) / mSection;
            mTexts[i] = String.valueOf((mMin + i * n)/20);
        }
...
//        }
         ...
         * 画实时度数值
         */
        mPaint.setColor(ContextCompat.getColor(getContext(), R.color.colorPrimaryMini));
        mPaint.setStrokeWidth(dp2px(2));
        int xOffset = dp2px(22);
        if (mVelocity >= 100) {
           drawDigitalTube(canvas, mVelocity/ 20, 0);
           // drawDigitalTube(canvas, (mVelocity - 110) / 40, 0);
